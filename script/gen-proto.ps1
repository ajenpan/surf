...
$protofiles | ForEach-Object {    
    ...
    & $protocbin --proto_path="$workDir"/tools/protobuf/include/ --proto_path=$protoPath --go_out=$goOutDir $_.FullName
    & $protocbin --proto_path="$workDir"/tools/protobuf/include/ --proto_path=$protoPath --csharp_out=$csharpOutDir --csharp_opt=file_extension=.pb.cs  $_.FullName
}
