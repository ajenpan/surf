...
func Test_SimpleWrite(t *testing.T) {
	...
	t.Run("no rotate", func(t *testing.T) {
		rl, err := NewRoteteLog("./testdata/test.log.2006010215")
...
func Test_Rotate(t *testing.T) {
	rl, err := NewRoteteLog("./testdata/test.log.2006010215", WithRotateTime(time.Hour))
	if assert.Empty(t, err) {
		defer rl.Close()
...
func Test_DeleteExpiredFile(t *testing.T) {
	...
	if assert.Empty(t, err) {
		defer rl.Close()
		for i := 0; i < 10; i++ {
			tempFile, _ := os.OpenFile(fmt.Sprintf("./testdata/test.log.%d", i), os.O_CREATE, 0644)
			tempFile.Close()
		}
